...
<font color='purple'> 0702价格添加询价,添加文件上传 </font>
...
#### 2. 添加商品
**权限：仅粮油公司组(`role=0`)**  
添加商品，添加时会传入一个价格，系统会用该价格创建当前周期以及结束日期在现在时间点之后的周期所使用的价格对象，并将价格对象的`status`设置为`1`（未审核）,自动生成价格请求.  
<font color='purple'> 0708 现在必须先有价格周期，才能创建商品</font>  
...
<font color=purple> 修改了返回字段，添加了询价 </font>  
...
<font color='purple'> 0708修改 需要传入周期ID，仅针对某一个周期进行报价修改 </font>  
...
#### 4. 批量批准价格请求  
<font color='purple'> 0702新增,通过传入价格对象id列表,批量批准价格请求 </font>  
需传入包含价格对象的id的列表
...
#### 5. 批量拒绝价格请求  
<font color='purple'> 0702新增,通过传入价格对象id列表,批量拒绝价格请求 </font>  
需传入包含价格对象的id的列表
